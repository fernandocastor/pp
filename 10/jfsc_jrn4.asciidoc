...
public class BathroomConditional {
  ...
  public void enterMale(){
    try{
      lck.lock();
    try {
      while (areYouThere) {conditional.await();}
    } catch (InterruptedException e) {
      e.printStackTrace();
    }
      areYouThere = true;
...
----------------------------------------------
Com a classe BathroomSync é possível fazer com que apenas uma pessoa do sexo oposto esteja no banheiro. Para tal, foi utilizado o mecanismo de exclusão mútua do metodo synchronized através de lock intríseco:
----------------------------------------------
...
synchronized (lock){
      try {
