...
vector<float> counter (NR_THREADS-1);
vector<ul> countermax (NR_THREADS-1);
ul globalcountmax = pow(2,26); // Limite superior antes de overflow do counter (float)
ul globalcount = 0;
ul globalreserve = 0;
vector<thread> counterp (NR_THREADS);
mutex gblcnt_mutex;

static void globalize_count ( float *counter, ul *countermax ) {
...
static void balance_count ( float *counter, ul *countermax ) {
...
bool add_count ( ul delta, float *counter, ul *countermax ) {
	cout << (*countermax - *counter) << " - " << delta << "\n";
